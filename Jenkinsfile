pipeline {
    ...
    parameters {
        ...
        string(name: 'SONAR_PROJECT_KEY', defaultValue: 'App', description: 'SonarQube Project Key')
        string(name: 'SONAR_PROJECT_NAME', defaultValue: 'Frontend-App', description: 'SonarQube Project Name')
        string(name: 'REPO_URL', defaultValue: 'https://github.com/Faikhan147/Real-Time-Frontend-App-Repo.git', description: 'Git repo URL')
    ...
    environment {
        ...
        APP_URL = credentials('frontend-app-url')
            ...
            steps {
                script {
                    ...
                    sh """
                        ...
                        helm template app-${params.ENVIRONMENT} ${HELM_CHART_DIR} || { echo 'Helm template failed!'; exit 1; }
                    """
                }
            }
        }
        ...
        script {
            ...
            retry(3) {
                withEnv(["APP_URL=$APP_URL"]) {
                    sh '''#!/bin/bash
                    echo "🌐 Performing app health check on $APP_URL ..."
                    sleep 10  # wait for service to be ready
                    echo "🔍 DNS resolution test:"
                    nslookup "$APP_URL" || echo "❗ DNS resolution failed."

                    echo "🔍 CURL with verbose output:"
                    curl -v "$APP_URL" || echo "❗ CURL failed."
    ...
    post {
        ...
        failure {
            script {
                ...
                def lastRevision = sh(script: "helm history app-${params.ENVIRONMENT} --namespace ${params.ENVIRONMENT} | tail -2 | head -1 | awk '{print \$1}'", returnStdout: true).trim()
                sh "helm rollback app-${params.ENVIRONMENT} ${lastRevision} --namespace ${params.ENVIRONMENT} || echo 'Rollback failed!'"
            }
        }
    }
}
